...
import { getRamayanaSargaVerses, getMahabharataChapterVerses } from '@/lib/scriptureReader';
...
export default function UnitScreen() {
  const { book, sectionKey, unitKey } = useLocalSearchParams<{
    book: string; sectionKey: string; unitKey: string;
  }>();
  ...
  const sargaCommentary = !loading && verses.length > 0
  ...
  const ListHeader = (
    <View>
      ...
      {sargaCommentary ? (
        <View style={[styles.contextCard, { borderColor: accent + '25', backgroundColor: accent + '05' }]}>
          ...
          renderItem={({ item: verse, index }) => (
            <TouchableOpacity
              ...
              onPress={() => router.push({
                ...
                params: { book, id: verse.id, sectionKey, unitKey, verseIndex: String(index) },
              })}
            >
              {/* Verse number */}
              ...
              <View style={styles.verseContent}>
                {verse.reference ? (
                  <Text style={[styles.verseRef, { color: accent + 'AA' }]} numberOfLines={1}>
                    {verse.reference}
                  </Text>
                ) : null}
...
const styles = StyleSheet.create({
  ...
  // ── Context card ──────────────────────────────────────────────────────
  contextCard: {
    marginHorizontal: Spacing.xl, marginTop: Spacing.xl, marginBottom: 8,
  ...
  unitHeader: {
    ...
    paddingRight: Spacing.xl, paddingVertical: 14,
  ...
  unitName: {
    ...
    color: Colors.text0, letterSpacing: 0.5, marginBottom: 3,
  },
  unitSub: {
